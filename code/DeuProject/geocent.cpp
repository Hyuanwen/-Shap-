...
DEUPROJECT_API int pj_geocentric_to_geodetic( double a, double es,
							                  ...
							                  double *x, double *y, double *z )
{
	...
	for( i = 0; i < point_count; i++ )
	{
		...
		pj_Convert_Geocentric_To_Geodetic( &gi, x[io], y[io], z[io], x+io, y+io, z+io );
	}

	return 0;
}

DEUPROJECT_API int pj_geodetic_to_geocentric( double a, double es,
											  long point_count, int point_offset,
											  double *x, double *y, double *z )
{
	double			b;
	...
	for( i = 0; i < point_count; i++ )
	{
		...
		if( pj_Convert_Geodetic_To_Geocentric( &gi, x[io], y[io], z[io], x+io, y+io, z+io ) != 0 )
		{
			pj_errno = -14;
			x[io] = y[io] = HUGE_VAL;
