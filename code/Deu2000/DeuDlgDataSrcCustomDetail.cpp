...
// CDeuDlgDataSrcCustomDetail 对话框
IMPLEMENT_DYNAMIC(CDeuDlgDataSrcCustomDetail, CXTResizeDialog)
CDeuDlgDataSrcCustomDetail::CDeuDlgDataSrcCustomDetail(CWnd* pParent /*=NULL*/)
	: CXTResizeDialog(CDeuDlgDataSrcCustomDetail::IDD, pParent)
{
...
CDeuDlgDataSrcCustomDetail::~CDeuDlgDataSrcCustomDetail()
{
}

void CDeuDlgDataSrcCustomDetail::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDeuDlgDataSrcCustomDetail, CXTResizeDialog)
	ON_BN_CLICKED(IDC_BTN_CLOSE, &CDeuDlgDataSrcCustomDetail::OnBnClickedBtnClose)
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_BTN_OK, &CDeuDlgDataSrcCustomDetail::OnBnClickedBtnOk)
END_MESSAGE_MAP()

BOOL CDeuDlgDataSrcCustomDetail::OnInitDialog()
{
	...
	if(_coordtype ==  GeoProjected) 
	{
		...
		{
			_ProjectedSeries p_Serises = CustDataMapping.ProjectedNode.pProjectedSeries.GetAt(n);
	...
	else if(_coordtype ==  GeoDetic)
	{
		...
		{
			_GeodeticSeries p_Serises = CustDataMapping.GeodeticNode.pGeodeticSeries.GetAt(n);
			if(p_Serises._name.text == m_Name)
			{
				nIndex = n; //记录索引
				m_wndPlaceHolder.ShowWindow(SW_HIDE);
...
void CDeuDlgDataSrcCustomDetail::OnBnClickedBtnClose()
{
	this->OnOK();	
}

void CDeuDlgDataSrcCustomDetail::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		CRect rctGrid, rctBtn;
		m_wndPropertyGrid.GetWindowRect(&rctGrid);
		m_btnOk.GetWindowRect(&rctBtn);
	
		int nHeight=0, nWidth=0;
		nHeight    = rctBtn.Height();
		nWidth	   = rctBtn.Width();
		
		rctBtn.right = rctGrid.right;
		rctBtn.left  = rctBtn.right-nWidth;
		rctBtn.top = rctGrid.bottom+10;
		rctBtn.bottom = rctBtn.top+nHeight;
		ScreenToClient(&rctBtn);
		m_btnOk.MoveWindow(rctBtn);

		m_btnOk.Invalidate();
	}	
}

void CDeuDlgDataSrcCustomDetail::OnBnClickedBtnOk()
{
	this->OnOK();
}
