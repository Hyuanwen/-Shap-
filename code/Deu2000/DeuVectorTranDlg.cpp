...
IMPLEMENT_DYNCREATE(CDeuVectorTranDlg, CDialog)
CDeuVectorTranDlg::CDeuVectorTranDlg()
	: CDialog(CDeuVectorTranDlg::IDD), m_Controlpoint_Dlg(VECTOR)
{
	iModel = MODEL_GRID;
	IsAdditional = FALSE;
...
void CDeuVectorTranDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDeuVectorTranDlg, CDialog)
	ON_WM_SIZE()	
	ON_WM_CREATE()
	ON_EN_SETFOCUS(IDC_EDIT_COORINATE_INPUT, &CDeuVectorTranDlg::OnCoordinateInput)
	ON_EN_SETFOCUS(IDC_EDIT_COORINATE_OUTPUT, &CDeuVectorTranDlg::OnCoordinateOutput)
	ON_BN_CLICKED(IDC_BTN_INPUT, &CDeuVectorTranDlg::OnBnClickedBtnInput)
	ON_BN_CLICKED(IDC_BTN_OUTPUT, &CDeuVectorTranDlg::OnBnClickedBtnOutput)
	ON_CBN_SELCHANGE(IDC_COMBO_TRANMODEL, &CDeuVectorTranDlg::OnCbnSelchangeComboTranmodel)
	ON_CBN_SELCHANGE(IDC_COMBO_INPUT, &CDeuVectorTranDlg::OnCbnSelchangeComboInput)
	ON_CBN_SELCHANGE(IDC_COMBO_OUTPUT, &CDeuVectorTranDlg::OnCbnSelchangeComboOutput)
	ON_MESSAGE(WM_PROGRESS, OnProgressStep)
	ON_WM_PAINT()	
	ON_BN_CLICKED(IDC_BUTTON1, &CDeuVectorTranDlg::OnBnClickedButton1)
END_MESSAGE_MAP()

// CDeuVectorTranDlg 诊断
//#ifdef _DEBUG
//void CDeuVectorTranDlg::AssertValid() const
//{
//	CDialog::AssertValid();
//}
//
//#ifndef _WIN32_WCE
//void CDeuVectorTranDlg::Dump(CDumpContext& dc) const
//{
//	CDialog::Dump(dc);
//}
//#endif
...
void CDeuVectorTranDlg::OnSize(UINT nType, int cx, int cy)
{
...
//int CDeuVectorTranDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
//{
...
//void CDeuVectorTranDlg::OnInitialUpdate()
//{
...
BOOL CDeuVectorTranDlg::OnInitDialog()
{
	...
	for(int n=0; n<_TranModel_Vector_num; n++)
	{
		m_ComboTranModel.InsertString(n, sz_TranModel_Vector[n]);
	}
...
void CDeuVectorTranDlg::OnBnClickedBtnInput()
{
	...
	{
		CDeuDlgFiles m_file(VECTOR, this);
...
void CDeuVectorTranDlg::OnBnClickedBtnOutput()
{
	CString strType;
...
void CDeuVectorTranDlg::OnCoordinateInput()
{
	/*
...
void CDeuVectorTranDlg::OnCoordinateOutput()
{
	/*
	CDeuDlgDataSrc m_DataSrcDlg;
	m_DataSrcDlg.DoModal();
...
//设置转换模型参数
void CDeuVectorTranDlg::OnCbnSelchangeComboTranmodel()
{
	CComboBox* p_combox3	  = (CComboBox*)GetDlgItem(IDC_COMBO_TRANMODEL);
...
//下拉框事件
void CDeuVectorTranDlg::OnCbnSelchangeComboInput()
{
	//ClearCache(FALSE);
	...
	//add by chenyong 2013-10-9 增加ArcSDE矢量转换	
	else if (strType == _T("ArcSDE"))
	{
		...
		::GetPrivateProfileString(_T("VECTORTRANSRC"), _T("SERVER"), _T(""), param.strServer.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANSRC"), _T("SERVICE"), _T(""), param.strService.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANSRC"), _T("DATABASE"), _T(""), param.strDatabase.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANSRC"), _T("USERNAME"), _T(""), param.strUsername.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANSRC"), _T("PASSWORD"), _T(""), param.strPassword.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANSRC"), _T("VERSION"), _T(""), param.strVersion.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		...
		if (conPro.DoModal() == IDOK)
		{
			...
			::WritePrivateProfileString(_T("VECTORTRANSRC"), _T("SERVER"), param.strServer, strRate);
			::WritePrivateProfileString(_T("VECTORTRANSRC"), _T("SERVICE"), param.strService, strRate);
			::WritePrivateProfileString(_T("VECTORTRANSRC"), _T("DATABASE"), param.strDatabase, strRate);
			::WritePrivateProfileString(_T("VECTORTRANSRC"), _T("USERNAME"), param.strUsername, strRate);
			::WritePrivateProfileString(_T("VECTORTRANSRC"), _T("PASSWORD"), param.strPassword, strRate);
			::WritePrivateProfileString(_T("VECTORTRANSRC"), _T("VERSION"), param.strVersion, strRate);
		}
...
//输出下拉框事件
void CDeuVectorTranDlg::OnCbnSelchangeComboOutput()
{
	//ClearCache(TRUE);
	...
	else if (strType == _T("ArcSDE"))
	{
		...
		::GetPrivateProfileString(_T("VECTORTRANDES"), _T("SERVER"), _T(""), param.strServer.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANDES"), _T("SERVICE"), _T(""), param.strService.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANDES"), _T("DATABASE"), _T(""), param.strDatabase.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANDES"), _T("USERNAME"), _T(""), param.strUsername.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANDES"), _T("PASSWORD"), _T(""), param.strPassword.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("VECTORTRANDES"), _T("VERSION"), _T(""), param.strVersion.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		...
		if (conDesPro.DoModal() == IDOK)
		{
			...
			::WritePrivateProfileString(_T("VECTORTRANDES"), _T("SERVER"), param.strServer, strRate);
			::WritePrivateProfileString(_T("VECTORTRANDES"), _T("SERVICE"), param.strService, strRate);
			::WritePrivateProfileString(_T("VECTORTRANDES"), _T("DATABASE"), param.strDatabase, strRate);
			::WritePrivateProfileString(_T("VECTORTRANDES"), _T("USERNAME"), param.strUsername, strRate);
			::WritePrivateProfileString(_T("VECTORTRANDES"), _T("PASSWORD"), param.strPassword, strRate);
			::WritePrivateProfileString(_T("VECTORTRANDES"), _T("VERSION"), param.strVersion, strRate);
		}
...
void CDeuVectorTranDlg::LoadExistData(CString m_str[10])
{
...
void CDeuVectorTranDlg::SetTransModel(const CString& strTransModel)
{
...
void CDeuVectorTranDlg::ClearCache(BOOL IsOut)
{	
	if(!IsOut)
	{
...
//转换结束通知消息面板
void CDeuVectorTranDlg::TranFinish(const char* strTimeMsg)
{
}

//转换结束通知消息面板
void CDeuVectorTranDlg::ShowStatusPanel(CString str)
{
...
BOOL CDeuVectorTranDlg::CheckControl()
{
	UpdateData();
...
LRESULT CDeuVectorTranDlg::OnProgressStep(WPARAM wParam, LPARAM lParam)
{
...
void CDeuVectorTranDlg::DisableAllController()
{
	CButton* p_btn1			  = (CButton*)GetDlgItem(IDC_BTN_INPUT);
	CButton* p_btn2			  = (CButton*)GetDlgItem(IDC_BTN_OUTPUT);
...
void CDeuVectorTranDlg::EnbleAllController()
{
	CButton* p_btn1			  = (CButton*)GetDlgItem(IDC_BTN_INPUT);
	CButton* p_btn2			  = (CButton*)GetDlgItem(IDC_BTN_OUTPUT);
...
//判断大地坐标是否存在
BOOL CDeuVectorTranDlg::CheckCoord()
{
...
void CDeuVectorTranDlg::OnPaint()
{
...
void CDeuVectorTranDlg::OnBnClickedButton1()
{

}

void CDeuVectorTranDlg::OnBnClickedRadio1()
{
	// TODO: 在此添加控件通知处理程序代码
}
...
void CDeuVectorTranDlg::InitTransFormat()
{
...
IWorkspacePtr CDeuVectorTranDlg::GetSrcWorkspace() const
{
	return m_pSrcWS;
}

IWorkspacePtr CDeuVectorTranDlg::GetDesWorkspace() const
{
	return  m_pDesWS;
}

vector<STR_SDEINFO> CDeuVectorTranDlg::GetSelVec() const
{
	return m_vecSelect;
}