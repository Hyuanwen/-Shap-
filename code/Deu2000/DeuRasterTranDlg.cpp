...
IMPLEMENT_DYNCREATE(CDeuRasterTranDlg, CDialog)
CDeuRasterTranDlg::CDeuRasterTranDlg()
	: CDialog(CDeuRasterTranDlg::IDD), m_Controlpoint_Dlg(RASTER)
{
	//this->SetScrollSizes(MM_TEXT,CSize(800,800));
	iModel = MODEL_GRID;			//默认转换模型
...
void CDeuRasterTranDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDeuRasterTranDlg, CDialog)
	ON_WM_SIZE()
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_WM_GETMINMAXINFO()
	ON_BN_CLICKED(IDC_BTN_INPUT, &CDeuRasterTranDlg::OnBnClickedBtnInput)
	ON_BN_CLICKED(IDC_BTN_OUTPUT, &CDeuRasterTranDlg::OnBnClickedBtnOutput)
	ON_EN_SETFOCUS(IDC_EDIT_COORINATE_INPUT, &CDeuRasterTranDlg::OnCoordinateInput)
	ON_EN_SETFOCUS(IDC_EDIT_COORINATE_OUTPUT, &CDeuRasterTranDlg::OnCoordinateOutput)
	ON_CBN_SELCHANGE(IDC_COMBO_TRANMODEL, &CDeuRasterTranDlg::OnCbnSelchangeComboTranmodel)
	ON_CBN_SELCHANGE(IDC_COMBO_TRANSFER, &CDeuRasterTranDlg::OnCbnSelchangeComboTransfer)
	ON_CBN_SELCHANGE(IDC_COMBO_INPUT, &CDeuRasterTranDlg::OnCbnSelchangeComboInput)
	ON_CBN_SELCHANGE(IDC_COMBO_OUTPUT, &CDeuRasterTranDlg::OnCbnSelchangeComboOutput)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_RECOLLECT, &CDeuRasterTranDlg::OnCbnSelchangeComboRecollect)
	ON_MESSAGE(WM_LOADWAIT, OnLoadWait)
	ON_BN_CLICKED(IDC_BTN_SETTING, &CDeuRasterTranDlg::OnBnClickedBtnSetting)
	ON_BN_CLICKED(IDC_RADIO_resample, &CDeuRasterTranDlg::OnBnClickedRadioResample)
	ON_BN_CLICKED(IDC_RADIO_transfer, &CDeuRasterTranDlg::OnBnClickedRadioTransfer)
END_MESSAGE_MAP()

// CDeuRasterTranDlg 诊断
//#ifdef _DEBUG
//void CDeuRasterTranDlg::AssertValid() const
//{
//	CDialog::AssertValid();
//}
//
//#ifndef _WIN32_WCE
//void CDeuRasterTranDlg::Dump(CDumpContext& dc) const
//{
//	CDialog::Dump(dc);
//}
//#endif
...
// CDeuRasterTranDlg 消息处理程序
void CDeuRasterTranDlg::OnSize(UINT nType, int cx, int cy)
{
...
//int CDeuRasterTranDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
//{
...
//void CDeuRasterTranDlg::OnInitialUpdate()
//{
...
void CDeuRasterTranDlg::OnDestroy()
{
	CDialog::OnDestroy();
}

BOOL CDeuRasterTranDlg::OnInitDialog()
{
	...
	//栅格转换模型
	for(int n=0; n<_TranModel_Raster_num; n++)
	{
		m_ComboTranModel.InsertString(n, sz_TranModel_Raster[n]);
	}
...
void CDeuRasterTranDlg::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
...
BOOL CDeuRasterTranDlg::CheckControl()
{
	...
	else if(m_strOutputFile.IsEmpty() && strType != _T("ArcSDE"))
	{
	...
	else if(!m_Controlpoint_Dlg.IsInputParams && this->m_Controlpoint_Dlg.m_List.GetItemCount() == 0)
	{
...
void CDeuRasterTranDlg::OnBnClickedBtnInput()
{
	/*CDeuDlgFiles m_file;
	...
	//else
	//{
		CDeuDlgFiles m_file(RASTER, this);
		...
		if(m_file.DoModal() == IDOK)
		{
			...
			if(_FilsList_selected.GetCount() > 0 )
			{
			    ...
			    hr2 = trans.FetchRasterProperty( _FilsList_selected.GetAt(0), &vFormat )  ;
				if(hr2 == S_OK)
				{
...
void CDeuRasterTranDlg::OnBnClickedBtnOutput()
{	
	//CString m_str;
...
void CDeuRasterTranDlg::OnCoordinateInput()
{
	/*
...
void CDeuRasterTranDlg::OnCoordinateOutput()
{
	/*
	CDeuDlgDataSrc m_DataSrcDlg;
	m_DataSrcDlg.DoModal();
...
//设置转换模型参数
void CDeuRasterTranDlg::OnCbnSelchangeComboTranmodel()
{	
	...
	CComboBox* p_cb_transfer	  = (CComboBox*)GetDlgItem(IDC_COMBO_TRANSFER);
	CString strTransfer;
	p_cb_transfer->GetWindowText(strTransfer);

	if( m_Controlpoint_Dlg.m_strModel == "格网转换模型" && m_bResample  )
	{		
		iModel=MODEL_GRID;
	}
	else if( m_Controlpoint_Dlg.m_strModel == "仿射转换模型" && m_bResample  )
	{		
		iModel=MOD_AFFINE;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "二阶多项式转换模型" && m_bResample )
	{	
		iModel=MOD_POLYNOMIAL2;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "三阶多项式转换模型" && m_bResample )
	{		
		iModel=MOD_POLYNOMIAL3;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "四阶多项式转换模型" && m_bResample )
	{		
		iModel=MOD_POLYNOMIAL4;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "布尔莎转换模型" && m_bResample )
	{		
		iModel=MODEL_BURSA;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "平面四参数转换模型" && m_bResample )
	{		
		iModel=MODEL_D2P4;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "格网转换模型" && !m_bResample && strTransfer == "全图点平移法" )
	{
		iModel=MODEL_TRANS_ALL_GRID;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "仿射转换模型" && !m_bResample && strTransfer == "全图点平移法" )
	{
		iModel=MODEL_TRANS_ALL_AFFINE;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "二阶多项式转换模型" && !m_bResample && strTransfer == "全图点平移法"  )
	{
		iModel=MODEL_TRANS_ALL_Poly2;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "三阶多项式转换模型" && !m_bResample && strTransfer == "全图点平移法" )
	{
		iModel=MODEL_TRANS_ALL_Poly2;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "四阶多项式转换模型" && !m_bResample && strTransfer == "全图点平移法" )
	{
		iModel = MODEL_TRANS_ALL_Poly2 ;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "布尔莎转换模型" && !m_bResample && strTransfer == "全图点平移法" )
	{
		iModel = MODEL_TRANS_ALL_BURSA ;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "平面四参数转换模型" && !m_bResample && strTransfer == "全图点平移法" )
	{
		iModel = MODEL_TRANS_ALL_D2P4 ;
	}
	else if (m_Controlpoint_Dlg.m_strModel == "格网转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel=MODEL_TRANS_QUICK_GRID;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "仿射转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel=MODEL_TRANS_QUICK_AFFINE;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "二阶多项式转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel=MODEL_TRANS_QUICK_Poly2;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "三阶多项式转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel=MODEL_TRANS_QUICK_Poly2;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "四阶多项式转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel = MODEL_TRANS_QUICK_Poly2 ;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "布尔莎转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel = MODEL_TRANS_QUICK_BURSA ;
	}
	else if(m_Controlpoint_Dlg.m_strModel == "平面四参数转换模型" && !m_bResample && strTransfer == "图廓点平移法" )
	{
		iModel = MODEL_TRANS_QUICK_D2P4 ;
	}
}

//设置转换模型参数
void CDeuRasterTranDlg::OnCbnSelchangeComboTransfer()
{	
	//m_ComboReCollect.GetWindowText(m_Controlpoint_Dlg.m_strModel);
...
//下拉框事件
void CDeuRasterTranDlg::OnCbnSelchangeComboInput()
{
	//add by chenyong 2013-10-10 for ArcSDE Raster
	CString strType;
	...
	if (strType == _T("ArcSDE"))
	{
		...
		::GetPrivateProfileString(_T("RASTERTRASRC"), _T("SERVER"), _T(""), param.strServer.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRASRC"), _T("SERVICE"), _T(""), param.strService.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRASRC"), _T("DATABASE"), _T(""), param.strDatabase.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRASRC"), _T("USERNAME"), _T(""), param.strUsername.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRASRC"), _T("PASSWORD"), _T(""), param.strPassword.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRASRC"), _T("VERSION"), _T(""), param.strVersion.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		...
		if (conPro.DoModal() == IDOK)
		{
			...
			::WritePrivateProfileString(_T("RASTERTRASRC"), _T("SERVER"), param.strServer, strRate);
			::WritePrivateProfileString(_T("RASTERTRASRC"), _T("SERVICE"), param.strService, strRate);
			::WritePrivateProfileString(_T("RASTERTRASRC"), _T("DATABASE"), param.strDatabase, strRate);
			::WritePrivateProfileString(_T("RASTERTRASRC"), _T("USERNAME"), param.strUsername, strRate);
			::WritePrivateProfileString(_T("RASTERTRASRC"), _T("PASSWORD"), param.strPassword, strRate);
			::WritePrivateProfileString(_T("RASTERTRASRC"), _T("VERSION"), param.strVersion, strRate);
...
void CDeuRasterTranDlg::OnCbnSelchangeComboOutput()
{
	//add by chenyong 2013-10-10
	CString strType;
	m_ComboOutput.GetWindowText(strType);
	...
	if (strType == _T("ArcSDE"))
	{
		...
		::GetPrivateProfileString(_T("RASTERTRADES"), _T("SERVER"), _T(""), param.strServer.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRADES"), _T("SERVICE"), _T(""), param.strService.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRADES"), _T("DATABASE"), _T(""), param.strDatabase.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRADES"), _T("USERNAME"), _T(""), param.strUsername.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRADES"), _T("PASSWORD"), _T(""), param.strPassword.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		::GetPrivateProfileString(_T("RASTERTRADES"), _T("VERSION"), _T(""), param.strVersion.GetBuffer(MAX_PATH), MAX_PATH, (LPCTSTR)strRate);
		conPro.SetParam(param);
		...
		if (conPro.DoModal() == IDOK)
		{
			...
			//保存配置信息
			param = conPro.GetParam();
			::WritePrivateProfileString(_T("RASTERTRADES"), _T("SERVER"), param.strServer, strRate);
			::WritePrivateProfileString(_T("RASTERTRADES"), _T("SERVICE"), param.strService, strRate);
			::WritePrivateProfileString(_T("RASTERTRADES"), _T("DATABASE"), param.strDatabase, strRate);
			::WritePrivateProfileString(_T("RASTERTRADES"), _T("USERNAME"), param.strUsername, strRate);
			::WritePrivateProfileString(_T("RASTERTRADES"), _T("PASSWORD"), param.strPassword, strRate);
			::WritePrivateProfileString(_T("RASTERTRADES"), _T("VERSION"), param.strVersion, strRate);
		}
...
//加载存在的参数
void CDeuRasterTranDlg::LoadExistData(CString m_str[10])
{
	CEdit* p_edit_input			= (CEdit*)GetDlgItem(IDC_EDIT_INPUT);
	CEdit* p_edit_output		= (CEdit*)GetDlgItem(IDC_EDIT_OUTPUT);
	CEdit* p_edit_incoord		= (CEdit*)GetDlgItem(IDC_EDIT_COORINATE_INPUT);
	...
	m_Controlpoint_Dlg.OnLoadControlData(m_str[6], m_str[7]);
	SetTransModel(m_str[7]);

	m_ComboInput.SetCurSel(m_ComboInput.FindString(0, m_str[8]));
	m_ComboOutput.SetCurSel(m_ComboOutput.FindString(0, m_str[9]));

	UpdateData(FALSE);
}

void CDeuRasterTranDlg::SetTransModel(const CString& strTransModel)
{
	m_ComboTranModel.SetCurSel(m_ComboTranModel.FindString(0, strTransModel));
	OnCbnSelchangeComboTranmodel();
	UpdateData(FALSE);
}

void CDeuRasterTranDlg::ClearCache(BOOL IsOut)
{	
	if(!IsOut)
	{
...
void CDeuRasterTranDlg::TranFinish(const char* strTimeMsg)
{
	//MessageBox("转换完毕！", "提示", MB_OK | MB_ICONINFORMATION);
}

//转换结束通知消息面板
void CDeuRasterTranDlg::ShowStatusPanel(CString str)
{
	((CDeuGlobalMainFrame*)theApp.m_pMainWnd)->SendMessage(WM_VEC_MSGPANEL, NULL, (LPARAM)str.GetBuffer());
}

LRESULT CDeuRasterTranDlg::OnProgressStep(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CDeuRasterTranDlg::OnLoadWait(WPARAM wParam, LPARAM lParam)
{
	unsigned int i = (unsigned int)lParam;
...
LRESULT CDeuRasterTranDlg::OnTranformFinsh(WPARAM wParam, LPARAM lParam)
{	
...
LRESULT CDeuRasterTranDlg::OnTranformFinshAll(WPARAM wParam, LPARAM lParam)
{	
	m_Progress.SetPos(100);
	m_Progress.SetWindowText("100%");
	EnbleAllController();
	MessageBox("转换完毕！", "提示",MB_OK | MB_ICONINFORMATION);
	return 0;
}
...
//转换结束，这里要兼容戴峰多线程的问题
void CDeuRasterTranDlg::TranformOver()
{
	MessageBox("转换完毕！", "提示",MB_OK | MB_ICONINFORMATION);
...
void CDeuRasterTranDlg::OnCbnSelchangeComboRecollect()
{
	UpdateData();
...
void CDeuRasterTranDlg::DisableAllController()
{
	CButton* p_btn1			  = (CButton*)GetDlgItem(IDC_BTN_INPUT);
	CButton* p_btn2			  = (CButton*)GetDlgItem(IDC_BTN_OUTPUT);
...
void CDeuRasterTranDlg::EnbleAllController()
{
	CButton* p_btn1			  = (CButton*)GetDlgItem(IDC_BTN_INPUT);
	CButton* p_btn2			  = (CButton*)GetDlgItem(IDC_BTN_OUTPUT);
...
//判断大地坐标是否存在
BOOL CDeuRasterTranDlg::CheckCoord()
{
...
void CDeuRasterTranDlg::OnBnClickedRadioResample()
{
	m_bResample = TRUE ;
...
void CDeuRasterTranDlg::OnBnClickedRadioTransfer()
{
	m_bResample = FALSE ;	
...
void CDeuRasterTranDlg::OnBnClickedBtnSetting()
{
...
void CDeuRasterTranDlg::InitTransFormat()
{
