...
void	CToolBar24::SetFullColorImage(UINT ID, COLORREF rgbBack, LPRECT lprect, int num)
{
	...
	m_imgToolbar.Create(lprect->right-5, lprect->bottom-5, ILC_COLOR24|ILC_MASK, 1, 1);
	...
	m_imgToolbarDis.Create(lprect->right-5, lprect->bottom-5, ILC_COLOR24|ILC_MASK, 1, 1);
	...
	m_imgToolbarNA.Create(lprect->right-5, lprect->bottom-5, ILC_COLOR24|ILC_MASK, 1, 1);
...
BOOL CToolBar24::OnEraseBkgnd(CDC* pDC) 
{
	...
	GetWindowRect(&rect);
	ScreenToClient(&rect);	
	CDC* pDC2=CDC::FromHandle(::GetDC(NULL));
...
{
	ID_BK=IDB_MAINFRAME_BACK;
	CBitmap tmp;
	tmp.LoadBitmap(ID_BK);
	brush.CreatePatternBrush(&tmp);
}

void CToolBar24::LoadBitmapBit24(UINT _ID)
{
	ID_BK=_ID/*IDB_BITMAP1*/;
	CBitmap tmp;
	tmp.LoadBitmap(ID_BK);
	brush.CreatePatternBrush(&tmp);
}